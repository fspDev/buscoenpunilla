...
export function OficioPropuestaCard({
  ...
  function handleAprobar() {
    startTransition(async () => {
      await aprobarOficioPropuesto(prestador_id, oficio_propuesto, nombreEditado)
      setDone(true)
    })
  }
  ...
  return (
    <div className={`rounded-xl border border-outline-variant bg-white p-4 shadow-card space-y-3 ${isPending ? 'opacity-60 pointer-events-none' : ''}`}>
      ...
      {/* Campo editable del oficio propuesto */}
      <div className="rounded-lg bg-surface px-3 py-2 space-y-1.5">
        <p className="text-xs text-on-surface-variant">Oficio propuesto</p>
        <p className="text-xs text-outline line-through">{oficio_propuesto}</p>
        <input
          value={nombreEditado}
        ...
        />
        {nombreEditado !== oficio_propuesto && nombreEditado.trim() && (
